    ...
    class(CAMBdata) :: this
    real(dl), intent(in) :: a
    real(dl) :: dtauda, grhoa2, grhov_t, ghroa_t

    call this%CP%DarkEnergy%BackgroundDensityAndPressure(this%grhov, a, grhov_t)
    !!Nhan!!
    call this%CP%Axion%BackgroundDensityAndPressure(a, ghroa_t)

    !  8*pi*G*rho*a**4.
    grhoa2 = this%grho_no_de(a) +  (grhov_t + ghroa_t) * a**2
    !!!!!!!!
    if (grhoa2 <= 0) then
    ...
    type EvolutionVars
        ...
        !!Nhan!!
        integer ax_ix !Index of two axion equations
        !!!!!!!!
    ...
    !Axion
    EV%ax_ix = neq + 1
    neq = neq + CP%Axion%num_perturb_equations
    maxeq = maxeq + CP%Axion%num_perturb_equations
    !!!!!!!!
    ...
    !!Nhan!!
    ! Axion
    if (CP%Axion%num_perturb_equations > 0) &
        yout(EVOut%ax_ix:EVOut%ax_ix + CP%Axion%num_perturb_equations - 1) = &
        y(EV%ax_ix:EV%ax_ix + CP%Axion%num_perturb_equations - 1)
    !!!!!!!!
    ...
    integer, parameter :: i_clxg=1,i_clxr=2,i_clxc=3, i_clxb=4, &
        i_qg=5,i_qr=6,i_vb=7,i_pir=8, i_eta=9, i_aj3r=10,i_clxde=11,i_vde=12,i_clxa=13,i_ua=14
    integer, parameter :: i_max = i_ua
    ...
    !!Nhan!!
    ! Axion: This initializes also i_va, when num_perturb_equations is set to 2.
    if (CP%Axion%num_perturb_equations > 0) then
        call CP%Axion%PerturbationInitial(InitVec(i_clxa:i_clxa + CP%Axion%num_perturb_equations - 1), &
            a, tau, k)
        y(EV%ax_ix:EV%ax_ix + CP%Axion%num_perturb_equations - 1) = &
            InitVec(i_clxa:i_clxa + CP%Axion%num_perturb_equations - 1)
    end if
    !!!!!!!!
	...
	!!Nhan!!
	real(dl) grhoa_t, gpres_ax, phi_ax, phidot_ax, dgrho_a, dgq_a
	!!!!!!!!
    ...
    !!Nhan!!
    call State%CP%Axion%BackgroundDensityAndPressure(a, grhoa_t, gpres_ax, phi_ax, phidot_ax)
    !!!!!!!!
    ...
    !!Nhan!!
    grho = grho_matter+grhor_t+grhog_t+grhov_t+grhoa_t
	gpres_noDE = gpres_nu + (grhor_t + grhog_t)/3 + gpres_ax
    !!!!!!!!
    ...
    !!Nhan!!
    ! Axion sources in Einstein eqs
    call State%CP%Axion%PerturbedStressEnergy(dgrho_a, dgq_a, a, grhoa_t, ay, ayprime, EV%ax_ix)
    dgrho = dgrho + dgrho_a
    dgq = dgq + dgq_a
    !!!!!!!!
    ...
    call State%CP%Axion%PerturbationEvolve(ayprime, EV%ax_ix, a, adotoa, k, z, ay, phi_ax, phidot_ax)
    !!!!!!!!
    ...
    if (EV%Evolve_baryon_cs) then
        ...
        !!Nhan!!
        delta_p_b = barssc0*(1._dl-0.75d0*State%CP%yhe+(1._dl-State%CP%yhe)*opacity*a2/(State%akthom/State%meR_func(a)**2))*Tmat*(clxb + delta_tm)
        !!!!!!!!
    else
        Delta_TM = clxg/4
        delta_p_b = cs2*clxb
    ...
    if (EV%Evolve_baryon_cs) then
        if (EV%Evolve_TM) then
            ...
            !!Nhan!!
            ayprime(EV%Tg_ix) = -2*k*(z+vb)/3 - a*  (Compton_CT/State%meR_func(a)**3) * (Trad**4) * xe / (1._dl+xe+State%fHe) * &
    ...
    if (associated(EV%OutputTransfer) .or. associated(EV%OutputSources)) then
        ...
        if (associated(EV%OutputTransfer)) then
            ...
            !!Nhan!!
            EV%OutputTransfer(Transfer_ax) = ay(EV%ax_ix)
            !!!!!!!!
    ...
    !!Nhan!!
    call CP%Axion%BackgroundDensityAndPressure(a, grhoa_t, gpres_ax)

    grho=grhob_t+grhoc_t+grhor_t+grhog_t+grhov_t+grhoa_t
    gpres=(grhog_t+grhor_t)/3._dl+grhov_t*w_dark_energy_t+gpres_ax
    !!!!!!!!
