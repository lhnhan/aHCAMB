    ...
    !!Nhan!!
    real(dl) a_tmp,cs2_tmp,opa_tmp,tauini,dellogtau
    integer plot_points
    !!!!!!!!
    ...
    if (EV%q_ix == 0) then
        ...
        plot_points = 30000
        tauini = 1._dl  ! initial tau to write
        dellogtau = (log(State%tau0)-log(tauini))/(plot_points-1)

        open(1, file='out_perturb.dat', status='REPLACE')
        print*, "Printing axion perturbations for wavenumber: ", EV%q

        tau = exp(log(tauini)-dellogtau)
        do j=1,plot_points
            if (j/=plot_points) then
                tauend=exp(log(tau)+dellogtau)
            else
                tauend=State%tau0
            end if
            call GaugeInterface_EvolveScal(EV,tau,y,tauend,tol1,ind,c,w)
            if (global_error_flag/=0) return
            call output(EV,y,0, tau,sources, 0)

            ! Printing \delta_a at each time step for one wavenumber k_plot
            call EV%ThermoData%Values(tauend,a_tmp,cs2_tmp,opa_tmp)
            write(1,*) a_tmp, y(EV%ax_ix)
        end do
