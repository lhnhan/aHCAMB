    ...
    allocate(TAxionStandard::P%Axion)
    !!!!!!!!
    ...
    !!Nhan!!
    character(LEN=:), allocatable :: NumStr, S, DarkEneryModel, RecombinationModel, AxModel
    !!!!!!!!
    logical :: DoCounts
    ...
    !!Nhan!!
    AxModel = UpperCase(Ini%Read_String_Default('axion_model', 'standard'))
    if (allocated(P%Axion)) deallocate(P%Axion)
    if (AxModel == 'STANDARD') then
        allocate (TAxionStandard::P%Axion)
	else if (AxModel == 'AXIHIGGS') then
        allocate (TAxiHiggs::P%Axion)
    else
        ErrMsg = 'Unknown axion model: '//trim(AxModel)
        return
    end if
    P%omah2 = Ini%Read_Double('omah2')
    call P%Axion%ReadParams(Ini)
    ! P%k_plot = Ini%Read_Double('plot_wavenumber')
    !!!!!!!!
    ...
    P%meR = Ini%Read_Double('me_ratio')
