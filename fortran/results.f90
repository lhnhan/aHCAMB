    ...
    type, extends(TCAMBdata) :: CAMBdata
		...
		!!Nhan!!
		real(dl), dimension(30000) :: sampled_a, phi_a, phidot_a, ddphi_a, ddphidot_a, omah2_func
		real(dl) :: m_a = 1.e-29_dl !axion mass in eV
		real(dl) :: phi_i = 0._dl !initial misalignment field value in units of (10^17 GeV)
		!!!!!!!!
...
10      if (this%CP%Omnuh2 < 1.e-7_dl) this%CP%Omnuh2 = 0
        ...
        !!Nhan!!
        this%Omega_de = 1 -(this%CP%omch2 + this%CP%ombh2 + this%CP%omnuh2 + this%CP%omah2)/h2 - this%CP%omk  &
		...
		!!Nhan!!
		! call this%CP%DarkEnergy%Copy(this)
        call this%CP%Axion%Init(this)

        !!Printing data!!
        ! open(1, file='out_background.dat', status='REPLACE')
        ! do i=1, 30000
        !     a_tmp = this%sampled_a(i)
        !     call this%CP%Axion%BackgroundDensityAndPressure(a_tmp, ghroa_t)
        !     call this%CP%Axion%HiggsVEV(a_tmp, this%CP%meR, vev)
        !     write(1,*) a_tmp, ghroa_t/a_tmp**2*grho_to_omega, (vev - 1)
        ! end do
        ! close(1)
    ...
    if (present(error)) then
		...
		!!Nhan!!
		write(*,'("m_e / m_e,0          = ",f9.6)') P%meR
		if (P%omah2 > 1e-8) then
			write(*,'("m_a             	= ",e9.2)') this%m_a
			write(*,'("Om_a h^2             = ",f13.10)') P%omah2
            write(*,'("phi_ini/GeV          = ", e13.5)') this%phi_i*(1.e17_dl)
		else
			write(*,'("Om_a h^2             = ",f13.10)') P%omah2
		end if
        ...
        write(*,'("Om_m (inc Om_u, exc Om_a)      = ",f9.6)') (P%ombh2+P%omch2+P%omnuh2)/h2
    ...
    !!Nhan!!
    real(dl) :: grhov_t, grhoa_t, rhonu, grhonu, a
    ...
    do i=1, n
        ...
        !!Nhan!!
        call this%CP%Axion%BackgroundDensityAndPressure(a, grhoa_t)
        !!!!!!!!
        grhonu = 0
    ...
    !!Nhan!!
    ddamping_da = (R**2 + 16*(1+R)/15)/(1+R)**2*dtauda(this,a)*a**2/(this%CP%Recomb%x_e(a)*(this%akthom/this%meR_func(a)**2))
    !!!!!!!!
    ...
    !ignoring reionisation, not relevant for distance measures
    !!Nhan!!
    noreion_doptdepth_dz = this%CP%Recomb%x_e(a)*(this%akthom/this%meR_func(a)**2)*dtauda(this,a)
    !!!!!!!!
    ...
    !!Nhan!!
    reion_doptdepth_dz = this%CP%Reion%x_e(z)*(this%akthom/this%meR_func(1._dl/(1._dl+z))**2)*dtauda(this,1._dl/(1._dl+z))
    !!!!!!!!

    end function reion_doptdepth_dz


    function meR_func(this, a) result(meR)
    class(CAMBdata) :: this
    real(dl), intent(in) :: a
    real(dl) meR

    call this%CP%Axion%HiggsVEV(a, this%CP%meR, meR)

    end function meR_func
    ...
    !!Nhan!!
    this%dotmu(1)=this%xe(1)*(State%akthom/State%meR_func(a0)**2)/a0**2
    !!!!!!!!
    ...
    do i=2,nthermo
        ...
        if (CP%Reion%Reionization .and. tau > State%reion_tau_start) then
            ...
            if (CP%Accuracy%AccurateReionization .and. CP%WantDerivedParameters) then
                !!Nhan!!
                this%dotmu(i)=(xe_a(i) - this%xe(i))*(State%akthom/State%meR_func(a)**2)/a2
                !!!!!!!!
        ...
        if (a*this%tb(i)-CP%tcmb < -1e-8) then
            !!Nhan!!
            dtbdla= dtbdla -(thomc0/State%meR_func(a)**3)*fe/adot*(a*this%tb(i)-CP%tcmb)/a**3
            !!!!!!!!
        end if
        ...
        !!Nhan!!
        this%dotmu(i)=this%xe(i)*(State%akthom/State%meR_func(a)**2)/a2
        !!!!!!!!
    ...
    if (CP%WantDerivedParameters) then
        associate(ThermoDerivedParams => State%ThermoDerivedParams)
            ...
            if (FeedbackLevel > 0) then
                ...
                write(*,'("zstar                = ",f10.4)') ThermoDerivedParams( derived_zstar )
                ...
                write(*,'("zdrag                = ",f10.4)') ThermoDerivedParams( derived_zdrag )
