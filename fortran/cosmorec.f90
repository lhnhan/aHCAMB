    ...
    !!Nhan!!
    integer, parameter :: Nz_H = 20000
    real(dl), parameter :: dz = (zmax-zmin)/(Nz_H-1)
    !!!!!!!!
    ...
    !!Nhan!!
    real(dl), allocatable :: zrec_tmp(:), mez(:)

	!external CosmoRec_calc_h_cpp
	external recfast_pp_cpp
	!!!!!!!!
    ...
    class is (CAMBdata)
        ...
        allocate(Hz(Nz_H), mez(Nz_H), zrec_tmp(Nz_H), zrec(Nz), xrec(Nz), tmrec(Nz), tmp(Nz))
        !!!!!!!!
        ...
        do i=1,Nz_H
            zrec_tmp(i) = zmax - (i-1)*dz
            if (i == Nz_H) zrec_tmp(Nz_H) = 0._dl
            Hz(i) = 1/dtauda(State,1/(1._dl+zrec_tmp(i)))*(1._dl+zrec_tmp(i))**2/MPC_in_sec
            mez(i) = State%meR_func(1/(1._dl+zrec_tmp(i)))
        end do


        !!!!!!!

        ! call MpiStop("Checking thermo finished!")
        ...
        !!Nhan!!
        call recfast_pp_cpp(OmegaB, State%CP%H0, State%CP%tcmb, State%CP%yhe, &
                        Nz, Nz_H, dz, zrec_tmp, Hz, mez, zrec, xrec, tmrec)
        !!!!!!!!
